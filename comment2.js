...
app.post('/comment', async (req, res) => {
    try {
        const newcomment = req.body; // Get the new video data from the request body

        // Validate if all required fields are provided
        if (!newcomment.videoId || !newcomment.title || !newcomment.description || !newcomment.uploader || !newcomment.videoUrl) {
            return res.status(400).send("Missing required fields");
        }

        // Insert the new video document into the 'videos' collection
        const result = await comment.insertOne(newVideo);
